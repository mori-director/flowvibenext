const fs = require('fs');
let code = fs.readFileSync('src/App_backup.tsx', 'utf-8');
...
code = code.replace(/const mermaidRef = useRef<HTMLDivElement>\(null\);/g, "");
code = code.replace(/const svgContainerRef = useRef<HTMLDivElement>\(null\);/g, "");
code = code.replace(/const \[zoom, setZoom\] = useState\(1\);[\s\S]*?const \[dragStart, setDragStart\] = useState\(\{ x: 0, y: 0 \}\);/g, "");

// 3. Update generateInitialFlow
const startRegex = /const generateInitialFlow = async \(\) => \{/;
const generateInitialFlowEndIndex = code.indexOf('const refineFlow = async () => {');
const oldGenerateInitialFlow = code.substring(code.search(startRegex), generateInitialFlowEndIndex);
...
[필수 지침]
1. 프론트엔드/UX 관점의 흐름만 작성하며, 백엔드/DB/이벤트 로직은 일반 프로세스나 판단(decision) 노드로 표현하세요.
2. 예외(어뷰징 등) 처리와 앱심사 지침 등을 분기로 표현해야 합니다.
3. 데이터 타입:
   - type: "startEnd" | "process" | "decision" | "database" | "external"
4. JSON 모델의 속성을 완벽히 맞추시오.\`;
      ...
      const response = await ai.models.generateContent({
        ...
        config: {
          ...
          responseSchema: {
            type: Type.OBJECT,
            properties: {
              analysis: { type: Type.STRING, description: "1. 업무 구조화 방향에 대한 기획자 요약" },
              nodes: { type: Type.ARRAY, items: { type: Type.OBJECT, properties: { id: {type: Type.STRING}, label: {type: Type.STRING}, type: {type: Type.STRING, description: "startEnd, process, decision, database, external 중 하나"} } } },
...
// 4. Update refineFlow
const refineStartRegex = /const refineFlow = async \(\) => \{/;
const refineEndIndex = code.indexOf('const copyToClipboard = async () => {');
const oldRefineFlow = code.substring(code.search(refineStartRegex), refineEndIndex);
...
기존과 동일한 JSON 포맷을 반환하되, '추가 요청사항'에 따라 노드와 엣지를 변경한 "전체" JSON 코드를 보내세요.\`;
...
// 5. Remove math and exports (which are huge) and replace downloadExport
const dlStartIndex = code.indexOf('const downloadExport = async (type: \'Figma\' | \'PPT\') => {');
const dlEndIndex = code.indexOf('return (', dlStartIndex);
if (dlStartIndex !== -1) {
    const downloadExportCode = code.substring(dlStartIndex, dlEndIndex);
    ...
    // Zoom/pan handles removed for reactflow natively handles it

    `;
    code = code.replace(downloadExportCode, newDownloadExport);
}

// 5.5 Remove old layout handles
code = code.replace(/const handleMouseDown = \([\s\S]*?\}\n    \}\n  \};\n/g, "");

// 6. Fix references to mermaidCode
...
// 7. Update View Area JSX format
const viewAreaStart = code.indexOf('{/* View Area */}');
const step3End = code.indexOf('{/* Background Decor */}');
if (viewAreaStart !== -1) {
    const rx = /\{\/\* View Area \*\/\}[\s\S]+?<\/div>[\s\n]*<\/div>[\s\n]*<footer/;
    ...
    code = code.replace(rx, newReactFlowJSX);
}

// Ensure there is no straggling unused functions
code = code.replace(/function analyzeEdgePath\([\s\S]*?\}\n\n  \/\/ ──/g, "// ──");
code = code.replace(/function inToEmu\([\s\S]*?\}\n\n  \/\/ ──/g, "// ──");
code = code.replace(/function makeCxnSpXml\([\s\S]*?\}\n\n  \/\/ ──/g, "// ──");
code = code.replace(/function px2in\([\s\S]*?\}\n\n  const downloadExport/g, "const downloadExport");

// Overwrite the real App.tsx
