...
export default function App() {
  ...
  const [history, setHistory] = React.useState<HistoryItem[]>(() => {
    const saved = localStorage.getItem("flowcraft_history");
...
위 추가 요청사항을 반영하여 완전히 수정된 새 JSON 구조체를 반환해줘.`;
  ...
  const saveToHistory = () => {
    ...
    localStorage.setItem("flowcraft_history", JSON.stringify(newHistory));
    alert("현재 플로우가 이력에 저장되었습니다.");
  };

  const deleteFromHistory = (id: string) => {
    if (!window.confirm("정말로 이 이력을 삭제하시겠습니까?")) return;
    ...
    <div className="h-screen w-screen bg-slate-50 flex flex-col font-sans overflow-hidden text-slate-900 relative">
      <nav className="bg-white/80 backdrop-blur-md border-b border-slate-200 sticky top-0 z-50 flex-none relative">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3 cursor-pointer" onClick={() => { setViewMode("editor"); setStep(1); }}>
          ...
          <div className="flex items-center gap-4">
            <div className="flex bg-slate-100 p-1 rounded-xl gap-1">
              ...
              <button onClick={() => setViewMode("history")} className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${viewMode === "history" ? "bg-white text-blue-600 shadow-sm" : "text-slate-500 hover:text-slate-700"}`}><HistoryIcon size={14} />Archive</button>
            </div>
          </div>
        </div>
      </nav>

      <main className="flex-1 overflow-hidden w-full max-w-7xl mx-auto flex flex-col h-full relative">
        {viewMode === "history" ? (
          <HistoryList items={history} onSelectItem={loadFromHistory} onPreviewItem={(item) => { setPreviewItem(item); setIsPreviewOpen(true); }} onDeleteItem={deleteFromHistory} />
        ) : (
          <div className="p-4 md:p-6 flex-1 flex flex-col overflow-hidden w-full h-full max-w-7xl mx-auto">
            {step === 1 && (
                  ...
                  <div className="flex-1 overflow-y-auto pr-2 flex flex-col gap-6 custom-scrollbar">
                      ...
                      <div>
                        ...
                        <div className="flex flex-wrap gap-1.5">
                          {["금융", "커머스", "공공", "통신", "제조", "헬스케어", "직접입력"].map((type) => (
                            <button key={type} onClick={() => setInfo((prev) => ({ ...prev, domain: type }))} className={`px-3 py-1.5 rounded-lg font-bold text-xs transition-all ${info.domain === type ? "bg-slate-900 text-white shadow-md" : "bg-slate-100 text-slate-500 hover:bg-slate-200"}`}>{type}</button>
                          ))}
                        </div>
                        {info.domain === "직접입력" && (
                          <input name="customDomain" value={info.customDomain} onChange={handleInfoChange} placeholder="도메인을 직접 입력 (예: 에듀테크)" className="w-full mt-2 px-3 py-2.5 rounded-xl bg-slate-50 border border-slate-100 focus:bg-white focus:border-blue-500 focus:ring-4 focus:ring-blue-100 outline-none transition-all font-semibold text-sm" />
                        )}
                      </div>
                      <div>
                        ...
                        <div className="flex flex-wrap gap-1.5">
                          {["Web", "App", "Admin", "Kiosk", "Event"].map((type) => (
                            <button key={type} onClick={() => setInfo((prev) => ({ ...prev, serviceType: type }))} className={`px-3 py-1.5 rounded-lg font-bold text-xs transition-all ${type === "Event" ? "hidden " : ""}${info.serviceType === type ? "bg-slate-900 text-white shadow-md" : "bg-slate-100 text-slate-500 hover:bg-slate-200"}`}>{type}</button>
                          ))}
                        </div>
                      </div>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        ...
                        <input name="serviceName" value={info.serviceName} onChange={handleInfoChange} placeholder="예: 우리은행 비대면 계좌개설" className="w-full px-3 py-2.5 rounded-xl bg-slate-50 border border-slate-100 focus:bg-white focus:border-blue-500 focus:ring-4 focus:ring-blue-100 outline-none transition-all font-semibold text-sm" />
                      </div>
                      <div>
                    ...
                    <div className="flex-1 flex flex-col min-h-[120px]">
                      <div className="flex items-center justify-between mb-1.5 flex-none">
                        ...
                        <button onClick={generateAutoDescription} disabled={isGeneratingDesc || !info.serviceName || !info.flowName || !info.serviceType} className="px-3 py-1.5 bg-blue-100 text-blue-700 hover:bg-blue-200 disabled:bg-slate-100 disabled:text-slate-400 flex items-center gap-1.5 transition-all rounded-lg text-xs font-black shadow-sm" title="SERVICE NAME, Flow Name, Channel을 기반으로 AI가 내용을 자동 완성합니다.">
                    ...
                    <div className="flex-1 flex flex-col min-h-[120px]">
                      ...
                      <textarea name="policy" value={info.policy} onChange={handleInfoChange} placeholder="참고할 법령, 서비스 정책, 이용약관 등을 입력하면 더 정확한 흐름을 제안합니다." className="w-full flex-1 px-3 py-2.5 rounded-xl bg-slate-50 border border-slate-100 focus:bg-white focus:border-blue-500 focus:ring-4 focus:ring-blue-100 outline-none transition-all resize-none font-semibold text-sm" />
                    </div>
                  </div>
                  {error && (
              ...
              <div className="h-full flex flex-col gap-4 animate-in fade-in slide-in-from-right-8 duration-500 overflow-hidden w-full max-w-7xl mx-auto">
                <header className="flex flex-col md:flex-row md:items-center justify-between gap-4 p-4 bg-white rounded-2xl shadow-sm border border-slate-200 flex-none">
                  <div>
                    ...
                    <p className="text-slate-500 text-sm mt-1">AI가 도출한 업무 플로우와 코드를 검토하고 필요시 수정하세요.</p>
                  </div>
                  <div className="flex items-center gap-3">
                    <div className="flex bg-slate-100 p-1 rounded-xl border border-slate-200">
                      <button onClick={() => { setLayoutDirection("TB"); try { const p = JSON.parse(jsonCode); p.layoutDirection = "TB"; setJsonCode(JSON.stringify(p, null, 2)); } catch(e){} }} className={`px-4 py-2 rounded-lg text-xs font-black transition-all ${layoutDirection === "TB" ? "bg-white text-blue-600 shadow-sm" : "text-slate-400 hover:text-slate-600"}`}>세로 (Vertical)</button>
                      <button onClick={() => { setLayoutDirection("LR"); try { const p = JSON.parse(jsonCode); p.layoutDirection = "LR"; setJsonCode(JSON.stringify(p, null, 2)); } catch(e){} }} className={`px-4 py-2 rounded-lg text-xs font-black transition-all ${layoutDirection === "LR" ? "bg-white text-blue-600 shadow-sm" : "text-slate-400 hover:text-slate-600"}`}>가로 (Horizontal)</button>
                    </div>
                    <button onClick={() => setStep(3)} className="px-6 py-3 bg-blue-600 text-white rounded-xl font-black hover:bg-blue-700 transition-all flex items-center justify-center gap-2 shadow-lg shadow-blue-200">최종 시각화 보기 <ChevronRight size={18} /></button>
                  </div>
                </header>
                {error && (<div className="p-4 bg-red-50 text-red-600 rounded-xl text-sm font-bold flex items-start gap-2 border border-red-100 flex-none"><AlertCircle size={18} className="shrink-0 mt-0.5" /> <div>{error}</div></div>)}
                <div className="grid grid-cols-1 lg:grid-cols-2 lg:grid-rows-1 gap-4 flex-1 min-h-0 h-full overflow-hidden">
                  ...
                  <div className="bg-slate-900 rounded-2xl shadow-xl overflow-hidden flex flex-col h-full">
                    ...
                    <textarea value={jsonCode} onChange={(e) => setJsonCode(e.target.value)} className="flex-1 h-full w-full p-6 font-mono text-xs text-emerald-400 bg-transparent outline-none resize-none leading-relaxed custom-scrollbar overflow-y-auto" spellCheck="false" />
                  </div>
                </div>
                <div className="bg-white rounded-2xl shadow-md border border-slate-200 p-3 flex flex-col md:flex-row gap-3 items-center flex-none w-full">
                  <div className="flex-1 w-full relative">
                    ...
                    <input type="text" value={refinePrompt} onChange={(e) => setRefinePrompt(e.target.value)} onKeyDown={(e) => e.key === "Enter" && refineFlow()} placeholder="예: 상품구입 프로세스를 도출하였으나 슬라이드로 밀어서 결제요청하기 기능 추가해줘" className="w-full pl-10 pr-4 py-3 rounded-lg bg-slate-50 border border-slate-100 focus:bg-white focus:border-blue-500 focus:ring-4 focus:ring-blue-100 outline-none transition-all font-medium text-xs" />
                  </div>
            ...
            {step === 3 && (
              <div className="h-full flex flex-col gap-4 animate-in fade-in slide-in-from-bottom-10 duration-700 overflow-hidden">
                  ...
                  <div className="p-5 border-b border-slate-100 flex flex-col md:flex-row md:items-center justify-between gap-4 bg-slate-50/30 flex-none">
                    ...
                    <div className="flex flex-col items-end gap-2">
                      <div className="flex flex-wrap gap-2">
                        <button onClick={() => setStep(2)} className="px-4 py-2.5 bg-white border border-slate-200 text-slate-700 rounded-xl font-bold hover:bg-slate-50 hover:border-blue-300 hover:text-blue-600 transition-all flex items-center gap-1.5 text-xs shadow-sm"><ArrowLeft size={16} /> 이전 단계로 (수정)</button>
                        <button onClick={saveToHistory} className="px-4 py-2.5 bg-blue-600 border border-blue-500 text-white rounded-xl font-black text-xs shadow-lg hover:bg-blue-700 transition-all flex items-center gap-2"><Save size={16} /> 작업 저장하기</button>
                      </div>
                    </div>
                  </div>
                ...
                <footer className="text-center text-slate-400 text-[11px] font-bold tracking-widest uppercase py-4 flex-none">FlowCraft Standard • Developed for Professional Service Planning</footer>
              </div>
            )}
          </div>
        )}
