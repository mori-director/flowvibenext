...
export default function App() {
  ...
  const handleSelectMenu = (id: string | null) => {
    ...
    if (menu.depth === 2 && menu.parentId) {
      ...
      if (parent && (!parent.flowData || !parent.flowData.structuredPlan || parent.flowData.structuredPlan.length === 0)) {
        showAlert(`${parent.name} 프로세스를 먼저 생성하세요.`, "계층 구조 확인");
        return;
      }
...
상위 설명 요약: ${parent.flowData.flowDesc}
상위 핵심 정책: ${parent.flowData.policy}

**[필수 엄격 지침 - 최우선 순위]**
1. 현재 작성하는 '${activeMenu.name}' 프로세스는 반드시 위 상위 프로세스의 정책과 서비스 방향을 100% 계승해야 합니다.
2. **[환각 방지]** 만약 '${activeMenu.name}' 메뉴가 상위 프로세스에서 '불가능'하거나 '제한'된 대상을 다룬다면(예: 14세 미만 가입 제한인데 '14세 미만 가입' 요청 시), 절대 가입 성공 프로세스를 만들지 마십시오. 대신 '가입 불가 안내/차단' 프로세스로 내용을 도출하십시오.
3. 어떤 경우에도 상위 프로세스의 로직과 충돌하거나 이를 벗어나는 독자적인 허용 정책을 수립할 수 없습니다.
4. 상위 단계의 프로세스를 '상세화(Detailing)'하거나 '구체화'하는 관점에서만 내용을 도출하십시오.`;
      }
    }
...
- 새로 추론하여 추가한 노드명/설명 끝에는 애스터리스크(*)를 붙이세요.`;
    ...
    if (activeMenu?.depth === 2 && activeMenu.parentId) {
      ...
      if (parent?.flowData) {
        parentContext = `
[부모 프로세스 설계 정보 (${parent.name})]
상위 설명: ${parent.flowData.flowDesc}
상위 구조: ${JSON.stringify(parent.flowData.structuredPlan.map(a => `${a.tag}: ${a.content}`))}
상위 정책: ${parent.flowData.policy}

**[핵심 도출 원칙 - 절대 준수]**
1. 하위 프로세스는 상위 프로세스의 흐름을 절대 뒤집거나 충돌할 수 없습니다. 상위에서 정의한 '제약 사항'은 하위에서도 '제약'으로 작용해야 합니다.
2. **[정책 불일치 대응]** 만약 현재 메뉴명('${activeMenu.name}')이 상위 정책상 금지된 행위를 암시한다면, '성공 흐름'이 아닌 '거절/안내/차단 흐름'을 다이어그램으로 그리십시오. (예: 14세 미만 가입 불가 정책인데 14세 미만 가입 메뉴라면, '가입 차단' 화면으로 종료)
3. 부모 단계에서 명시된 정책과 화면 흐름의 큰 틀을 유지하면서, '${activeMenu.name}'과 관련된 세부 절차와 예외 케이스를 상세화하십시오.
4. 상위 프로세스에서 언급된 명칭과 용어를 일관되게 사용하세요.`;
      }
    }
...
위 추가 요청사항을 반영하여 완전히 수정된 새 JSON 구조체를 반환해줘.`;
  ...
  const handleAddMenu = (name: string, parentId?: string) => {
    ...
    if (parentId) {
      ...
      if (parent && (!parent.flowData || !parent.flowData.structuredPlan || parent.flowData.structuredPlan.length === 0)) {
        showAlert(`${parent.name} 프로세스를 먼저 생성한 후 하위 메뉴를 추가할 수 있습니다.`, "하위 메뉴 추가 제한");
        return;
      }
    }
